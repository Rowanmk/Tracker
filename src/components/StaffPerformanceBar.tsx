import React, { useState, useEffect } from 'react';
    ...
    export const StaffPerformanceBar: React.FC<StaffPerformanceBarProps> = ({
      ...
      const { dashboardMode, setDashboardMode, viewMode, setViewMode } = useDashboardView();
        ...
        const fetchTotalTarget = async () => {
          ...
          try {
            ...
            if (dashboardMode === "individual" && currentStaff) {
              const { totalTarget: individualTarget } = await loadTargets(selectedMonth, selectedFinancialYear, currentStaff.staff_id);
              combinedTarget = individualTarget;
            } else {
      ...
      }, [dashboardMode, currentStaff?.staff_id, selectedMonth, selectedFinancialYear, effectiveStaffPerformance.length]);
      ...
      const handleDashboardModeChange = (mode: "team" | "individual") => {
      ...
      return (
        <div className="w-full py-4 bg-[#001B47] rounded-xl flex justify-between items-center px-6">
          ...
          {/* Right: Dashboard mode dropdown and View mode dropdown */}
          <div className="flex items-center space-x-3">
            <select
              value={dashboardMode}
              onChange={(e) => handleDashboardModeChange(e.target.value as "team" | "individual")}
              disabled={loading}
              className="bg-white text-gray-900 px-3 py-2 rounded-md shadow-sm text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
            >
