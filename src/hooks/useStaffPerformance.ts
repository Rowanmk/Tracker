...
export const useStaffPerformance = (sortMode: SortMode): UseStaffPerformanceResult => {
  ...
  const fetchPerformanceData = useCallback(async () => {
    ...
    try {
      ...
      const isFullTeamSelection = !selectedTeamId || selectedTeamId === "team-view";
      ...
      const previousYear = selectedMonth === 1 ? yearForSelectedMonth - 1 : yearForSelectedMonth;
      ...
      const staffResults: StaffPerformance[] = filteredStaff.map((staff) => {
        ...
        const previousMonthTotal = staffPrevActivities.reduce((sum, a) => {
          if (a.service_id !== BAGEL_SERVICE_ID) return sum + (a.delivered_count || 0);
          return sum;
        }, 0);
  ...
  useEffect(() => {
    const handler = () => { void fetchPerformanceData(); };
  ...
  return { staffPerformance: performanceData, dailyActivities, teamTarget, loading, error, refetch: fetchPerformanceData };
};