...
export const useStaffPerformance = (sortMode: SortMode): UseStaffPerformanceResult => {
  ...
  const fetchPerformanceData = useCallback(async () => {
    ...
    try {
      ...
      // Filter staff by selected team if applicable
      ...
      // 1. Fetch Activities for current month
      ...
      // 2. Fetch Historical Data for the FY
      ...
      const staffResults: StaffPerformance[] = await Promise.all(
        filteredStaff.map(async (staff) => {
          ...
          const prevMonthTotal = staffPrevActivities.reduce((s, a) => s + a.delivered_count, 0);
  ...
  }, [authLoading, servicesLoading, allStaff, selectedMonth, selectedYear, financialYear, sortMode, selectedTeamId]);
